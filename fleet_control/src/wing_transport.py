...
def go_to_rack(args):
    ...
    pos_acc = 0.01
    ang_acc = 0.1
    print "base_command(%f, %f, %f, %f, %f, %s, %s)" % (base_x, base_y, theta, pos_acc, ang_acc, False, "/nu_rack")
    base_command(base_x, base_y, theta, pos_acc, ang_acc, False, "/nu_rack")
...
def grasp_wing_side_pose(args):
    # Arm posture command
    for i in range(3):
        arm_command_wrapper('grasp_ladder_side')
        rospy.sleep(0.1)
...
def arm_feedback_to_grasp_wing(args):
    ...
    arm_feedback_wrapper(base_x, base_y, theta, pos_acc, ang_acc, False, "/nu_rack")
