...
robot = dict()
robot['carry'] = ['drc1', 'drc2', 'drc3']
# robot['coarse_perception'] = ['drc2']
...
usevisinsp = False
new_2nd_hole_alignment = False
skewed = False

# all_robots = ['drc3', 'drc2', 'drc1', 'drc4', 'master']
# all_robots = ['drc3', 'drc1', 'drc2', 'master']
all_robots = ['drc2', 'drc3', 'drc1', 'master']


pregrasp_radius = 1.1
lidar_orientation = np.pi/2
grasp_radius_intermediate = 0.25
# grasp_radius = 0.20 # With finger
grasp_radius = 0.15 # without finger
...
# camera_robot = 'drc2'
# camera_poses = {
#     'away': ['/ladder', -2.5, 0.0, 0.0],
#     'view_ladder': ['/ladder',-0.928, 0.671, -0.368],
#     'inspect_ladder': ['/ladder', -1.110, -0.072, -0.092]
# }
...
def tuck_arms():
    ...
    fcmd.data = 12.0 # tuck arms for 6.5 sec
...
def rotate_to_face_ladder():
    ...
    ps = PointStamped()
    ps.header.frame_id = '/ladder'
    ps = transform_by_subjects(ps, '/youbot_triangle')
    ...
    local_fleet_pose = get_fleet_origin_in_frame('/youbot_triangle')
    qt = euclid.Quaternion(local_fleet_pose.pose.orientation.w,
                           local_fleet_pose.pose.orientation.x,
                           local_fleet_pose.pose.orientation.y,
                           local_fleet_pose.pose.orientation.z)
...
def rotate_into_ladder_alignment():
    ...
    if new_2nd_hole_alignment:
        ...
        if skewed:
            yaw_offset = np.pi/360.0*4.0
        else:
            yaw_offset = -np.pi/360.0*70.3
        wing_radius = 0.35
        #wing_radius = 0.70
...
def establish_fleet_formation():
    ...
    origin.pose = vicon_to_geometry_pose('/youbot_triangle')
...
ack_carry2 = AckHandler([robot['carry'][2]])
...
try:
    ...
    distributed_base_cmd('arm_feedback_to_grasp_wing_side', 0.0, grasp_radius, duration = 2.0)
    ...
    while (time.time()-start_tuck_arms) < 7:
    # busy waiting to make sure tucking ends.
        time.sleep(0.5)

    raw_input('Hit enter to open grippers and terminate program.')

    distributed_cmd('open_gripper',[robot['carry'][0]],ack_carry0)
    distributed_cmd('open_gripper',[robot['carry'][1]],ack_carry1)
    distributed_cmd('open_gripper',[robot['carry'][2]],ack_carry1)
