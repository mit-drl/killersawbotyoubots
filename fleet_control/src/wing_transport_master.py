...
camera_robot = 'drc2'
camera_poses = {
    'away': ['/ladder', -1.95, 0.0, 0.0],
    'view_hole1': ['/ladder', -0.956, 0.538,-0.372],
    # 'view_ladder's: ['/ladder',-1.033, 0.410, -0.163],
    # 'inspect_ladder': ['/ladder', -1.110, -0.072, 0.4]
    'inspect_ladder': ['/ladder', -1.133, -0.036, 0.106],
    'inspect_hole1': ['/ladder',  1.115,  0.702, -2.554],
    'inspect_hole2': ['/ladder', -1.156,  0.738, -0.372],
    'inspect_hole3': ['/ladder', -1.133, -0.036,  0.106],
    'inspect_hole4': ['/ladder',  1.228,  0.037, -3.048],
    'inspect_hole4_after': ['/ladder',  1.228 + 0.5,  0.037, -3.048],
    'inspect_hole_via1': ['/ladder',  0.591, 1.531, -2.853],
    'inspect_hole_via2': ['/ladder',  -1.085, 1.429, -1.847],
    'inspect_hole_via1_rotated': ['/ladder',  0.591, 1.531, 0.],
    'inspect_hole_via2_rotated': ['/ladder',  -1.085, 1.429, 0.]
}
...
def get_input(timeout=10.0):
    ...
    with sr.Microphone() as source: # use the default microphone as the audio source
        try:
            print 'listen'
            audio = r.listen(source, timeout) # listen for the first phrase and extract it into audio data
            print audio
        except Exception:
            print 'exception'
        ...
        try:
            ...
            text = r.recognize(audio, True) # recognize speech using Google Speech Recognition
            return text
        except LookupError: # speech is unintelligible
            print("Could not understand audio")
            return None

def wait_for_speech(text):
    if isinstance(text, basestring):
        text = [text] #wrap the string in a list if it's not a list
    print "Waiting for input of "
    print text
    if text == 'continue':
        timeout = 100.0
    else:
        timeout = 9.0
    for i in range(2):
        data = get_input(timeout)
        print data
        if data is None:
            text_to_speech['master'].publish("I didn't copy that.")
            rospy.sleep(1.0)
            continue
        for elem in data:
            speech = string.lower(elem['text'])
            if 'pause' in speech and not text == 'continue':
                print 'pause'
                text_to_speech['master'].publish('Paused.')
                wait_for_speech('continue')
                print 'continue'
                text_to_speech['master'].publish('Continuing.')
                return
            for t in text:
                if t in speech:
                    return t
        ...
        rospy.sleep(1.0)
    #failed to hear it:
    text_to_speech['master'].publish('Proceeding but could not verify command.')
    global voice
    voice = False
...
def lower_wing_skin_2():
    ...
    fleet_pose.pose.position.z = 0.057
    # or try getting it on /map and setting to 0.24
    frame = '/ladder'
    lin_vel = 0.05
    ang_vel = 0.05
...
def lower_wing_skin_3():
    ...
    #fleet_pose.pose.position.z = fleet_pose.pose.position.z - 0.04
    #fleet_pose.pose.position.z = 0.063
    fleet_pose.pose.position.z = 0.058
    # or try getting it on /map and setting to 0.24
    frame = '/ladder'
    lin_vel = 0.05
    ang_vel = 0.05
...
def move_camera_robot_to_inspect_hole1():
    set_camera_pose(camera_poses['inspect_hole1'])

def move_camera_robot_to_inspect_hole2():
    set_camera_pose(camera_poses['inspect_hole2'])

def move_camera_robot_to_inspect_hole3():
    set_camera_pose(camera_poses['inspect_hole3'])

def move_camera_robot_to_inspect_hole4():
    set_camera_pose(camera_poses['inspect_hole4'])

def move_camera_robot_to_inspect_hole4_after():
    set_camera_pose(camera_poses['inspect_hole4_after'])

def move_camera_robot_to_inspect_hole_via1():
    set_camera_pose(camera_poses['inspect_hole_via1'])

def move_camera_robot_to_inspect_hole_via2():
    set_camera_pose(camera_poses['inspect_hole_via2'])

def move_camera_robot_to_inspect_hole_via1_rotated():
    set_camera_pose(camera_poses['inspect_hole_via1_rotated'])

def move_camera_robot_to_inspect_hole_via2_rotated():
    set_camera_pose(camera_poses['inspect_hole_via2_rotated'])

def move_camera_robot_away():
    set_camera_pose(camera_poses['away'])

def move_camera_robot_to_view_hole1():
    set_camera_pose(camera_poses['view_hole1'])
...
cmds = { 'lift_wing_skin': lift_wing_skin,
         ...
         'move_camera_robot_to_inspect_hole1': move_camera_robot_to_inspect_hole1,
         'move_camera_robot_to_inspect_hole2': move_camera_robot_to_inspect_hole2,
         'move_camera_robot_to_inspect_hole3': move_camera_robot_to_inspect_hole3,
         'move_camera_robot_to_inspect_hole4': move_camera_robot_to_inspect_hole4,
         'move_camera_robot_to_inspect_hole4_after': move_camera_robot_to_inspect_hole4_after,
         'move_camera_robot_to_inspect_hole_via1': move_camera_robot_to_inspect_hole_via1,
         'move_camera_robot_to_inspect_hole_via2': move_camera_robot_to_inspect_hole_via2,
         'move_camera_robot_to_inspect_hole_via1_rotated': move_camera_robot_to_inspect_hole_via1_rotated,
         'move_camera_robot_to_inspect_hole_via2_rotated': move_camera_robot_to_inspect_hole_via2_rotated,
         'move_camera_robot_away': move_camera_robot_away,
         # 'move_camera_robot_to_view_ladder': move_camera_robot_to_view_ladder,
         'move_camera_robot_to_view_hole1': move_camera_robot_to_view_hole1,
...
def run_visinsp(target):
    ...
    # init_pose = mat_to_pose(pose_to_mat(init_pose), pose_to_mat(delta))
     # get from ICP result
    delta_mat = np.matrix([[0.990142, -0.0616429, 0.12577, -0.0157555],
        [0.07617, 0.990537, -0.114173, -0.0039241],
        [-0.117542, 0.122627, 0.985468, 0.0344625],
        [0, 0, 0, 1]])
    delta_mat = np.linalg.inv(delta_mat)

    delta_all = mat_to_pose(pose_to_mat(delta), delta_mat)
    init_pose = mat_to_pose(pose_to_mat(init_pose), pose_to_mat(delta_all))
...
try:
  ...
  if do_first_hole:
      if usekinect:
        ...
        if usevisinsp:
            centralized_cmd('move_camera_robot_to_inspect_hole1', 'move camera robot to the initial position')
            distributed_cmd('hole1_inspection_pose', robot['coarse_perception'], ack_coarse, duration=10.0)
          # centralized_cmd('move_camera_robot_to_view_hole1', 'move camera robot to view ladder')
          # distributed_cmd('arm_inspection_config_tilted', robot['coarse_perception'], ack_coarse, duration=2.0)
        ...
        if voice:
            raw_input('enter')
            wait_for_speech('demo') #start the demo
            text_to_speech['master'].publish('Starting.')
      ...
      if voice:
          text_to_speech['master'].publish('Shall I proceed with finding the hole?')
          wait_for_speech('hole') #find the hole
      ...
      if usehokuyo:
          ...
          hole_detection_one_acc += distributed_cmd('search_ladder', robot['fine_perception'], ack_fine, 'Robot 2 finding Hole on Ladder.', duration=5.0, bubbles=False)
          ...
          rospy.sleep(4.0) #Added to stop speech from clobbering each other
      ...
      if voice:
          text_to_speech['master'].publish('Robots in position.  Should we grab the panel?')
          wait_for_speech('grab') #find the hole
      ...
      if voice:
          text_to_speech['master'].publish('We are holding the panel.  Should we transport it to the ladder?')
          wait_for_speech('transport') #find the hole
          #text_to_speech['master'].publish('Carry the panel.')
      ...
      if usehokuyo:
          ...
          while not rospy.is_shutdown():
              ...
              #raw_input('search_skin done. hit enter')
              ...
              #raw_input('prealign_holes done. hit enter')
              ...
              #raw_input('align_holes done. hit enter')
      ...
      centralized_cmd('lower_wing_skin_2', robot['carry'])
      hole_detection_carry_acc += centralized_cmd('pause_fleet',robot['fine_perception'],  'Robot 2 can confirm the panel is in Position.')
      if voice:
          rospy.sleep(4.0)
          text_to_speech['master'].publish("Should I insert the fastener?")
          wait_for_speech('insert') #insert the fastener
      distributed_insert_cmd('Insert', robot['fine_perception'], ack_fine, -np.pi/2.0, 'hole_1', 'Inserting fastener.')
      #distributed_cmd('back_away_slightly',robot['fine_perception'],ack_fine)
      ...
      while not rospy.is_shutdown():
        if usevisinsp:
          text_to_speech['master'].publish('Robot 4, verify fastener number 1 attachment.')
          #raw_input("Hit enter to run visual inspection.\n")
          if run_visinsp('fastner1'):
            text_to_speech['master'].publish('Fastener attached successfully.  Proceeding to next hole.')
            break
          else:
            text_to_speech['master'].publish('Fastener attachment failed.')
            if voice:
                text_to_speech['master'].publish('Can you confirm the failure?')
                hrd = wait_for_speech(['yes', 'no'])
                if hrd == 'no':
                    break
                else:
                    text_to_speech['master'].publish('Retrying insertion.')
        else:
          break
        if usehokuyo:
            hole_detection_one_acc += distributed_cmd('search_ladder', robot['fine_perception'], ack_fine, 'Robot 2 finding Hole on Ladder.', duration=5.0, bubbles=False)
        ...
        if voice:
            rospy.sleep(4.0)
            text_to_speech['master'].publish('Robot 2 can confirm the panel is in Position.  Should I insert the fastener?')
            wait_for_speech('insert') #insert
        distributed_insert_cmd('Insert', robot['fine_perception'], ack_fine, -np.pi/2.0, 'hole_1', 'Inserting fastener.')
        #distributed_cmd('back_away_slightly',robot['fine_perception'],ack_fine)
        distributed_insert_cmd('LiftTool', robot['fine_perception'], ack_fine, 0., 'hole_1', "Fastener number 1 installed.", bubbles=False)
  ...
  # if usevisinsp:
    ...
    # distributed_cmd('arm_inspection_config', robot['coarse_perception'], ack_coarse, duration=2.0)
  if voice:
    text_to_speech['master'].publish('Is the fastener reloaded?')
    wait_for_speech('reloaded')
  #############BEGIN HOLE 2 ##################
  ...
  if new_2nd_hole_alignment:
      ...
      #skewed = 3.0
      #centralized_cmd('rotate_into_ladder_alignment', speech='Robots 1 and 3 macro-aligning the panel')
      vicon_acc +=  distributed_cmd('align_with_hole_2', robot['fine_perception'], ack_fine, 'Robot 2 is checking hole position.', bubbles=False)

      if usekinect and usevisinsp:
        centralized_cmd('move_camera_robot_to_inspect_hole_via1', 'move camera robot to inspect fastener number 2')
        centralized_cmd('move_camera_robot_to_inspect_hole_via2', '')
        centralized_cmd('move_camera_robot_to_inspect_hole2', '')
        distributed_cmd('hole2_inspection_pose', robot['coarse_perception'], ack_coarse, duration=10.0)
      #if usehokuyo:
      #    hole_detection_other_acc += distributed_cmd('search_ladder_2_bottom', robot['fine_perception'], ack_fine, bubbles=False)
      #skewed = -23.0
      #centralized_cmd('rotate_into_ladder_alignment', speech='Robots 1 and 3 macro-aligning the panel')
      distributed_cmd('open_gripper',[robot['carry'][1]],ack_carry1)
      #distributed_cmd('compliant_grip',[robot['carry'][1]],ack_carry1)
      distributed_cmd('slide_pose',[robot['carry'][1]],ack_carry1)
      ...
      #distributed_cmd('back_away_slightly',robot['fine_perception'],ack_fine)
      distributed_insert_cmd('LiftTool', robot['fine_perception'], ack_fine, 0., 'hole_2', 'Fastener inserted.', bubbles=False)

      while not rospy.is_shutdown():
        if usevisinsp:
          #raw_input("Hit enter to run visual inspection.\n")
          text_to_speech['master'].publish('Robot 4, verify fastener number 2 attachment.')
      ...
      #distributed_cmd('back_away_right',[robot['carry'][0]],ack_carry0)
      #distributed_cmd('back_away_right',[robot['carry'][1]],ack_carry1)
  else:
      ...
      while not rospy.is_shutdown():
        ...
        if voice:
          rospy.sleep(4.0)
          text_to_speech['master'].publish('Robot 2 can confirm the panel is in Position.  Should I insert the fastener?')
          wait_for_speech('insert') #find the hole
        distributed_insert_cmd('Insert', robot['fine_perception'], ack_fine, -np.pi/2.0, 'hole_2', 'Panel aligned to second hole.  Inserting fastener.', bubbles=False)
  ...
  if voice:
    ...
    wait_for_speech('reloaded')
  ###############BEGIN HOLE 3####################
    #text_to_speech['master'].publish('Hole 3 or hole 4 next?')


  do = '3'
  vi = '2'
  if voice:
    #heard = wait_for_speech(['three', 'four', '3', '4'])
    heard = '3'
    if heard == 'three' or heard == '3':
        do = '3'
        vi = '2'
    else:
        do = '4'
  ...
  distributed_cmd('back_away_right',robot['fine_perception'],ack_fine)
  distributed_base_cmd('go_to_ladder', -np.pi/2.0, prepush_radius, robot['fine_perception'], ack_fine, bubbles=False)

  if usekinect and usevisinsp:
    # centralized_cmd('move_camera_robot_to_inspect_ladder', 'move camera robot to inspect ladder')
    centralized_cmd('move_camera_robot_to_inspect_hole3', 'move camera robot to inspect fastener number 3')
    distributed_cmd('hole3_inspection_pose', robot['coarse_perception'], ack_coarse, duration=10.0)

  while not rospy.is_shutdown():
    vicon_acc +=  distributed_cmd('align_with_hole_' + do, robot['fine_perception'], ack_fine, 'Robot 2 is checking hole position.', bubbles=False)

    if usehokuyo:
      hole_detection_other_acc += distributed_cmd('search_ladder_' + do, robot['fine_perception'], ack_fine, bubbles=False)
      hole_detection_other_acc += distributed_cmd('search_ladder_fine_' + do, robot['fine_perception'], ack_fine, bubbles=False)

    if voice:
          text_to_speech['master'].publish('Robot 2 can confirm the panel is in Position.  Should I insert the fastener?')
          wait_for_speech('insert') #insert fastener
    distributed_insert_cmd('Insert', robot['fine_perception'], ack_fine, np.pi/2.0, 'hole_' + do, 'Hole Aligned.  Inserting Fastener.', bubbles=False)

    #distributed_cmd('LiftTool', robot['fine_perception'], ack_fine, fastener inserted.  Proceeding to hole 4.)
    distributed_insert_cmd('LiftTool', robot['fine_perception'], ack_fine, 0., 'hole_' + do,  'Fastener inserted.', bubbles=False)

    if usevisinsp:
      #raw_input("Hit enter to run visual inspection.\n")
      text_to_speech['master'].publish('Verifying fastener attachment.')
      if run_visinsp('fastner' + vi):
            text_to_speech['master'].publish('Fastener attached successfully.  Proceeding to next hole.')
            break
      else:
            text_to_speech['master'].publish('Fastener attachment failed.')
  ...
  if usekinect and usevisinsp:
    distributed_cmd('back_away',robot['coarse_perception'],ack_coarse)
    distributed_cmd('back_away',robot['coarse_perception'],ack_coarse)
    centralized_cmd('move_camera_robot_to_inspect_hole_via2_rotated', 'move camera robot to inspect fastener number 4')
    centralized_cmd('move_camera_robot_to_inspect_hole_via1_rotated', '')
    centralized_cmd('move_camera_robot_to_inspect_hole4', '')
    distributed_cmd('hole4_inspection_pose', robot['coarse_perception'], ack_coarse, duration=10.0)
  ...
  #distributed_cmd('LiftTool', robot['fine_perception'], ack_fine, "Fastener is inserted.  Job's finished.")
    ...
    distributed_insert_cmd('LiftTool', robot['fine_perception'], ack_fine, 0., 'hole_' + do, "Fastener is inserted.  ", bubbles=False)

    if usevisinsp:
      #raw_input("Hit enter to run visual inspection.\n")
      text_to_speech['master'].publish('Robot 4, verify final fastener attachment.')
      if run_visinsp('fastner' + vi):
            text_to_speech['master'].publish('Fastener attached successfully.  Job is finished.')
            break
      else:
            text_to_speech['master'].publish('Fastener attachment failed.')
            if voice:
  ...
  if usekinect and usevisinsp:
    centralized_cmd('move_camera_robot_to_inspect_hole4_after', '')

  # Move robot for next run.
  #distributed_cmd('back_away',robot['fine_perception'],ack_fine)
  #distributed_cmd('back_away_right',robot['fine_perception'],ack_fine)
  #distributed_base_cmd('go_to_ladder', np.pi/2.0, prepush_radius, robot['fine_perception'], ack_fine, bubbles=False)
