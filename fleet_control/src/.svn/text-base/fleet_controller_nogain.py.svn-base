...
CMD_TIMEOUT = 0.1
FORMATION_GAIN = 0.05 # 0.2 #1.0
FORMATION_GAIN_Z = 0.05 # 0.3 #0 #0.4
FORMATION_GAIN_W = 0.05 # 0.2 #1.0
...
# midpoint of all the arms and which has a yaw such that the frame is aligned
...
def get_mean_pose(names):
    ...
    # Yaw in global frame
    yaw = math.atan2(sumpt.y - my_arm_pose.trans.y, sumpt.x - my_arm_pose.trans.x)
    #print "\t%s %s %f" % (my_name, sumpt, yaw)
    ...
    arm = global_frame_to_body_frame(my_arm_pose)
    #if my_name == 'drc3':
...
def handle_set_fleet(msg):
    ...
    while True:
        mean_pose = get_mean_pose(arms_list)
