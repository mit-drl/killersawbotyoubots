...
grasp_radius = 0.363 ## from hand to center
...
try:
  ...
  if do_first_hole:
      ...
      distributed_joint_cmd('set_joint_5', angle=0.0)
      raw_input('enter')
      ...
      text_to_speech['master'].publish('We are holding the panel.  Should we carry it to the ladder?')
      if voice:
          wait_for_speech('carry') #find the hole
      ...
      centralized_cmd('lower_wing_skin_2', robot['carry'])
      hole_detection_carry_acc += centralized_cmd('pause_fleet',robot['fine_perception'],  'Robot 2 can confirm the panel is in Position.  Should I insert the fastener?')
      if voice:
      ...
      while not rospy.is_shutdown():
        if usevisinsp:
          ...
          if run_visinsp('fastner1'):
            text_to_speech['master'].publish('Robot 4 confirms fastener number 1 is attached successfully.')
            break
          else:
      ...
      while not rospy.is_shutdown():
        if usevisinsp:
          ...
          if run_visinsp('fastner3'): # fastner3 because we changed the order of insertions.
            text_to_speech['master'].publish('Robot 4 confirms fastener number 2 is attached successfully. Proceeding to hole 3.')
            break
          else:
      ...
      while not rospy.is_shutdown():
        ...
        if usevisinsp:
          ...
          if run_visinsp('fastner3'):
            text_to_speech['master'].publish('Robot 4 confirms fastener number 2 is attached successfully.')
            break
          else:
            text_to_speech['master'].publish('Fastener number 2 attachment failed. Retrying insertion.')
        else:
  ...
  while not rospy.is_shutdown():
    ...
    if usevisinsp:
      #raw_input("Hit enter to run visual inspection.\n")
      text_to_speech['master'].publish('Verifying fastener number attachment.')
      if run_visinsp('fastner' + vi):
        text_to_speech['master'].publish('Fastener attached successfully.  Proceeding to final hole.')
        break
      else:
        text_to_speech['master'].publish('Fastener attachment failed. Retrying insertion.')
    else:
      break
  ...
  #distributed_cmd('LiftTool', robot['fine_perception'], ack_fine, "Fastener is inserted.  Job's finished.")
    ...
    if usevisinsp:
      ...
      if run_visinsp('fastner' + vi):
        text_to_speech['master'].publish('Robot 4 confirms fastener is attached successfully. Assembly complete.')
        break
      else:
        text_to_speech['master'].publish('Fastener attachment failed. Retrying insertion.')
    else:
